...
const Year2024 = () => {
    ...
    const count1Ref = useRef(null); // 309,025
    const count2Ref = useRef(null); // 14
    const count3Ref = useRef(null); // ~3,000
    const count4Ref = useRef(null); // 100%
    const count5Ref = useRef(null); // 6
    const count6Ref = useRef(null); // 0
    const labelRefs = [
        useRef(null),
        useRef(null),
        useRef(null),
    ...
    return (_jsx("div", { className: "w-full flex justify-center bg-white overflow-hidden", children: _jsx("div", { className: "w-full max-w-7xl mx-auto", children: _jsxs("section", { ref: sectionRef, className: "w-full py-8 sm:py-10 md:py-12 lg:py-16 px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16 flex flex-col items-center gap-8 md:gap-10", children: [_jsxs("div", { className: "flex flex-col items-center", style: { width: "fit-content" }, children: [_jsxs("h2", { ref: titleRef, className: "text-5xl md:text-6xl lg:text-7xl font-bold text-center leading-tight", style: { opacity: 0 }, children: [_jsx("span", { style: { color: "var(--text-purple-dark, #5e2a7c)" }, children: "Year " }), _jsx("span", { style: { color: "var(--text-yellow, #f5a623)" }, children: "2024" })] }), _jsx("div", { ref: dotlineRef, className: "-mt-2", style: {
                                    opacity: 0,
                                    height: "6px",
                                    width: "100%",
                                    backgroundImage: "radial-gradient(circle, var(--text-purple-dark, #5e2a7c) 3px, transparent 1.5px)",
                                    backgroundSize: "10px 6px",
                                } })] }), _jsxs("div", { className: "w-full flex flex-col lg:flex-row items-center lg:items-stretch justify-center gap-8 md:gap-10 lg:gap-12", children: [_jsx("div", { className: "flex-shrink-0 flex justify-center items-start", children: _jsx("img", { ref: imageRef, src: girlImage, alt: "Girl smiling", className: "rounded-2xl object-cover", style: {
                                        opacity: 0,
                                        width: "clamp(200px, 50vw, 300px)",
                                        height: "clamp(280px, 60vw, 400px)",
                                        boxShadow: "0 8px 30px rgba(0,0,0,0.15)",
                                    } }) }), _jsx("div", { className: "flex-1 w-full grid grid-cols-1 sm:grid-cols-2 gap-y-8 md:gap-y-10 gap-x-6 md:gap-x-8", children: stats.map((stat, i) => {
                                    var _a, _b;
                                    return (_jsxs("div", { className: "flex flex-col gap-1 text-center", children: [_jsxs("span", { ref: stat.ref, className: "font-bold text-center", style: {
                                                    color: stat.color,
                                                    opacity: 0,
                                                    fontSize: "clamp(2rem, 6vw, 3.5rem)",
                                                    lineHeight: 1.1,
                                                }, children: [(_a = stat.prefix) !== null && _a !== void 0 ? _a : "", "0", (_b = stat.suffix) !== null && _b !== void 0 ? _b : ""] }), _jsx("p", { ref: labelRefs[i], className: "text-xs sm:text-sm md:text-base font-medium text-center max-w-xs mx-auto", style: { color: "#333", opacity: 0 }, children: stat.label })] }, i));
                                }) })] })] }) }) }));
};
